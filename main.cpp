...
#define の準備を始める start
#define あとはセバスチャンにお任せします false

こちらの main 様は 以下のことをなさいます
    くらすたあ わたくし("ここにトークンを置くのですわ！");

    わたくし.のスラッシュコマンドが実行されたら([](自動判別するですわ こまんど) 以下のことをしますですわ
        もし (こまんど.コマンドデータ.コマンドの名前を取得しますわ == "ping") なら以下のことをしますですわ
            こまんど.お返事を送信("ぽん！")ですわ
        以上ですわ
    以上ですわ);

    わたくし.が準備完了になったら([&わたし](が自動判別するですわ いべんと) 以下のことをしますですわ
        もし (一回も実行されていないのでしたら<データの塊 多数のコマンドたち>()) が誠なら以下のことをしますですわ
            わたくし.全世界にコマンドを知らしめますわ(
                スラッシュコマンド("ping", "Ping pong!", わたくし.の.個人識別IDを取得しますですわ)
            );
        標準出力 に "準備完了ですわ！" を 表示する ですわ
        以上ですわ
    以上ですわ);

    わたくし.の準備を始める(あとはセバスチャンにお任せします)ですわ
